...
already_built=$(($(curl --silent -I https://public-files.orthanc.team/tmp-builds/nightly-osx-builds/$last_commit_id/$first_artifact | grep -E "^HTTP"     | awk -F " " '{print $2}') == 200))
...
if [[ $already_built == 0 ]]; then
    ...
    aws s3 --region eu-west-1 cp /tmp/artifacts/ s3://public-files.orthanc.team/tmp-builds/nightly-osx-builds/ --recursive --cache-control=max-age=1
