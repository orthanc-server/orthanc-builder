...
if [[ $target == "orthanc" ]]; then
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd $buildRootPath
        cmake $framework_flags -DALLOW_DOWNLOADS=ON -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF -DPYTHON_VERSION=3.14 $sourcesRootPath
        make -j 4
    ...
    if [[ $dl != 0 ]]; then
        ...
        cmake $framework_flags -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DALLOW_DOWNLOADS=ON -DCMAKE_BUILD_TYPE:STRING=Release -DUSE_SYSTEM_GOOGLE_TEST=ON -DUSE_SYSTEM_ORTHANC_SDK=OFF -DUSE_SYSTEM_BOOST=OFF $sourcesRootPath/MySQL
    ...
    if [[ $dl != 0 ]]; then
        ...
        # CHANGE_VERSION_STL
        # wget https://orthanc.uclouvain.be/downloads/linux-standard-base/orthanc-stl/1.4/dist.zip --output-document dist.zip --quiet
        wget https://public-files.orthanc.team/lsb-mirror/STL-dist-1.4.zip --output-document dist.zip --quiet
        unzip dist.zip
    ...
    if [[ $dl != 0 ]]; then
        ...
        pushd /target
        # this is used by the ARM64 build
        tar -zcvf $buildRootPath/stone.wasm.tar.gz StoneWebViewer/
        # this is used by the macOS build
        zip -r $buildRootPath/wasm-binaries.zip StoneWebViewer/
